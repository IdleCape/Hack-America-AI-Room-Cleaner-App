// ════════════════════════════════════════════════════════════════
// cleaner.js — React-powered Room Cleaner component
// Depends on: React, ReactDOM (CDN), SharedBells (shared.js)
// ════════════════════════════════════════════════════════════════
...
const iconCamera = React.createElement("svg",
...
const iconUpload = (color) => React.createElement("svg",
  { width:22, height:22, viewBox:"0 0 24 24", fill:"none", stroke:color, strokeWidth:2, strokeLinecap:"round", strokeLinejoin:"round" },
...
function RoomCleaner() {
  ...
  const analyzeImages = async (frames, previewUrl) => {
    ...
    try {
      ...
      const imageContent = resized.map(b64 => ({
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...imageContent,
              { type: "text", text: frames.length > 1
                ? `These are ${frames.length} frames extracted from a room video. Analyze them together and provide cleaning instructions.`
                : "Analyze this room photo and provide cleaning instructions." }
            ]
          }]
        })
      });
  ...
  const handleFile = async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      const img = new Image();
      img.onload = async () => {
        const canvas = document.createElement("canvas"), MAX = 1280;
        let w = img.width, h = img.height;
        if (w > MAX || h > MAX) {
  ...
  const detectedCategories = analysis
    ? CLUTTER_CATEGORIES.filter(c => analysis.categories.includes(c.id))
    : [];
  ...
  if (screen === "home") return React.createElement("div", { style:{ paddingTop:8 } },
    React.createElement("div", { style:{ marginBottom:24 } },
      ...
      React.createElement("p",  { style:{ fontSize:13, color:"#888", marginTop:4 } },
        "Scan your room — every cleaning step earns 🪙 bells to spend in the Decorate game!")
    ),
    React.createElement("div", { style:{ display:"flex", flexDirection:"column", gap:12 } },
      React.createElement("button",
        { onClick:()=>{ setMode("camera"); setScreen("camera"); }, className:"btn-primary", style:{ display:"flex", alignItems:"center", gap:12, justifyContent:"center" } },
        iconCamera, "Use camera"
      ),
      React.createElement("button",
        { onClick:()=>{ setMode("upload"); setScreen("upload"); }, style:{ width:"100%", padding:"18px 20px", fontSize:15, fontWeight:600, borderRadius:12, background:"#fff", color:"#1a1a1a", border:"0.5px solid rgba(0,0,0,0.15)", cursor:"pointer", display:"flex", alignItems:"center", gap:12, justifyContent:"center" } },
        iconUpload("#1a1a1a"), "Upload photo or video"
      )
    ),
    React.createElement("div", { style:{ marginTop:20, padding:"14px 16px", background:"#fff8e6", border:"1px solid #c28b00", borderRadius:12, fontSize:13, color:"#7a5a00" } },
      ...
      " Scan your room, start cleaning, and tick off each cleaning step as you go. Each step earns ",
      React.createElement("strong", null, "+50 🪙 bells"),
      " you can spend in the 🪑 Decorate tab to furnish your dream room!"
    )
  );
  ...
  if (screen === "upload") return React.createElement("div", null,
    ...
    React.createElement("div", {
      ...
      React.createElement("svg", { width:44, height:44, viewBox:"0 0 24 24", fill:"none", stroke: dragging?"#3b8c56":"#ccc", strokeWidth:1.5, strokeLinecap:"round", strokeLinejoin:"round", style:{ marginBottom:12 } },
        React.createElement("rect",     { x:3, y:3, width:18, height:18, rx:2 }),
        React.createElement("circle",   { cx:8.5, cy:8.5, r:1.5 }),
        React.createElement("polyline", { points:"21 15 16 10 5 21" })
      ),
      React.createElement("p", { style:{ fontSize:15, fontWeight:500, color: dragging?"#3b8c56":"#555", marginBottom:4 } }, dragging ? "Drop it here!" : "Click to browse or drag a file here"),
    ...
    React.createElement("div", { style:{ padding:"12px 14px", background:"#e8f5ed", borderRadius:8, fontSize:13, color:"#3b8c56", marginBottom:12 } },
  ...
  if (screen === "results" && analysis) return React.createElement("div", null,
    ...
    videoFrames.length > 1
      ? React.createElement("div", { style:{ marginBottom:16 } },
          ...
          React.createElement("div", { style:{ display:"flex", gap:4, overflowX:"auto", paddingBottom:4 } },
            videoFrames.map((f, i) =>
              React.createElement("img", { key:i, src:f, onClick:()=>setCurrentFrame(i), style:{ height:60, borderRadius:6, border: i===currentFrame?"2px solid #3b8c56":"2px solid transparent", cursor:"pointer", flexShrink:0, objectFit:"cover", width:80 } })
            )
          ),
    ...
    detectedCategories.length > 0 && React.createElement("div", { style:{ marginBottom:16 } },
      ...
      React.createElement("div", { style:{ display:"flex", flexWrap:"wrap", gap:6 } },
        detectedCategories.map(cat =>
          React.createElement("span", { key:cat.id, style:{ padding:"4px 10px", borderRadius:20, fontSize:12, fontWeight:600, background:cat.bg, color:cat.color } }, cat.label)
        )
      )
    ),
    ...
    React.createElement("div", { style:{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 } },
      analysis.steps.map((step, i) => {
        ...
        return React.createElement("div", {
          key: i, onClick: () => toggleStep(i),
          style:{ display:"flex", alignItems:"flex-start", gap:12, padding:"12px 14px", background: done?"#EAF3DE":"#ffffff", border:"0.5px solid "+(done?"#97C459":"rgba(0,0,0,0.1)"), borderRadius:10, cursor:"pointer" }
        },
          React.createElement("div", { style:{ flexShrink:0, width:22, height:22, borderRadius:"50%", border:"1.5px solid "+(done?"#639922":"#ccc"), background: done?"#639922":"transparent", display:"flex", alignItems:"center", justifyContent:"center", marginTop:1 } },
            done && React.createElement("svg", { width:12, height:12, viewBox:"0 0 12 12", fill:"none" },
              React.createElement("path", { d:"M2 6l3 3 5-5", stroke:"white", strokeWidth:1.5, strokeLinecap:"round", strokeLinejoin:"round" })
            )
          ),
    ...
    React.createElement("div", { style:{ display:"flex", gap:8 } },
      ...
      React.createElement("button", {
        ...
        style:{ flex:1, padding:12, fontSize:14, fontWeight:600, borderRadius:12, background:"#3b8c56", color:"#fff", border:"none", cursor:"pointer" }
      }, "Scan again")
    )
  );
